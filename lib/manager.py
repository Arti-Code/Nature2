    ...
    def draw_net2(self, network: Network) -> Surface:
        if not network:
            ...
            for layer in network.layers:
                ...
                n = 0
                base_line.append(round((cfg.NET_BASE + max_nodes_num * v_space)/2))
            back_box:Rect = Rect(4, cfg.SCREEN[1] - (max(base_line))-4, max_net_length+105, max_layer_size+6)

            net_surf: Surface = Surface((back_box.w, cfg.SCREEN[1]))
            net_surf.fill(Color(0, 0, 0))
            gfxdraw.circle(net_surf, 100, 100, 50, Color(0,255,0))
            gfxdraw.aapolygon(net_surf, [back_box.topleft, back_box.topright, back_box.bottomright, back_box.bottomleft], Color(0, 255, 255))
            pygame.draw.polygon(net_surf, Color(0, 255, 255), [(back_box.left+1, back_box.top+1), (back_box.right-2, back_box.top+1), (back_box.right-2, back_box.bottom-2), (back_box.left+1, back_box.bottom-2)], 1)
            gfxdraw.filled_polygon(net_surf, [back_box.topleft, back_box.topright, back_box.bottomright, back_box.bottomleft], Color(0,0,255, 50))
            for node_key in network.layers[layer].nodes:
                node: Node = network.nodes[node_key]
                v = node.value
                if node.recurrent:
